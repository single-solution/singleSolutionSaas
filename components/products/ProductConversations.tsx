...
import type { ProductConversation, ProductConversationStatus, ProductConversationSummary } from "@/lib/types";

const THREAD_POLL_MS = 6000;

const STATUS_FILTERS: { value: ProductConversationStatus | "all"; label: string }[] = [
  { value: "all", label: "All" },
...
const STATUS_TONE: Record<ProductConversationStatus, "brand" | "neutral" | "success"> = {
  open: "brand",
...
export function ProductConversations({
  ...
  const [statusFilter, setStatusFilter] = useState<ProductConversationStatus | "all">("all");
  ...
  const loadList = useCallback(async () => {
    ...
    setLoadingList(true);
    try {
      const response = await platformApi.listProductConversations(siteId, productSlug, {
        status: statusFilter === "all" ? undefined : statusFilter,
        pageSize: 50,
      });
      setConversations(response.conversations);
    } catch (caughtError) {
      setListError(caughtError instanceof PlatformApiError ? caughtError.message : "Could not load conversations.");
    } finally {
  ...
  const loadThread = useCallback(
    async (conversationId: string, { silent = false }: { silent?: boolean } = {}) => {
      ...
      try {
        const response = await platformApi.getProductConversation(siteId, productSlug, conversationId);
        setThread(response.conversation);
      } catch (caughtError) {
        if (!silent) {
          setThreadError(caughtError instanceof PlatformApiError ? caughtError.message : "Could not load conversation.");
        }
      } finally {
  ...
  async function handleSend(event: FormEvent) {
    ...
    setSending(true);
    try {
      const response = await platformApi.sendProductConversationMessage(siteId, productSlug, selectedId, body);
      setThread(response.conversation);
      setReply("");
      void loadList();
    } catch (caughtError) {
      ...
      <div className="grid min-h-[32rem] grid-cols-1 md:grid-cols-[20rem_1fr]">
        <aside className="flex flex-col border-b border-line md:border-b-0 md:border-r">
          <div className="flex items-center justify-between gap-2 border-b border-line px-4 py-3">
              ...
              {STATUS_FILTERS.map((filter) => (
                <button
                  ...
                  className={cn(
                    ...
                    statusFilter === filter.value ? "bg-brand-600 text-white" : "text-ink-muted hover:bg-surface-subtle",
                  )}
                >
                  {filter.label}
                </button>
            ...
            >
              <RefreshCw className={cn("h-4 w-4", loadingList && "animate-spin")} aria-hidden="true" />
            </button>
          </div>

          <div className="flex-1 overflow-y-auto">
            {loadingList ? (
              <p className="px-4 py-6 text-sm text-ink-muted" aria-live="polite">
              ...
              <EmptyState icon={Inbox} title="No conversations" description="Customer chats will appear here." />
            ) : (
                  ...
                  <li key={conversation.id}>
                    <button
                      ...
                      className={cn(
                        ...
                        selectedId === conversation.id ? "bg-surface-subtle" : "hover:bg-surface-subtle",
                      )}
                    >
                      ...
                      <p className="truncate text-[13px] text-ink-muted">{conversation.lastMessagePreview}</p>
                      <div className="flex items-center gap-2">
                        <Badge tone={STATUS_TONE[conversation.status]}>{conversation.status}</Badge>
                        <span className="text-xs text-ink-faint">{formatTime(conversation.lastMessageAt)}</span>
                      </div>
                    </button>
                  </li>
                ))}
        ...
        <section className="flex min-h-[32rem] flex-col">
          {!selectedId ? (
            <div className="flex flex-1 items-center justify-center p-6">
              <EmptyState icon={MessagesSquare} title="Select a conversation" description="Choose a chat on the left to read and reply." />
            </div>
          ) : loadingThread ? (
            ...
            <Alert tone="danger" title="Load failed" className="m-4">
              {threadError}
              <div className="mt-3">
                <Button variant="outline" size="sm" onClick={() => selectedId && void loadThread(selectedId)}>
                  Retry
                </Button>
              </div>
            </Alert>
            ...
            <>
              <div className="flex items-center justify-between gap-2 border-b border-line px-5 py-3">
                <div className="min-w-0">
                  <p className="truncate font-semibold text-ink">{thread.customerName}</p>
                  {thread.assistantPaused ? (
                    <p className="text-xs text-ink-muted">Assistant paused — handed to a human</p>
                  ) : null}
                </div>
                <Badge tone={STATUS_TONE[thread.status]}>{thread.status}</Badge>
              </div>

              <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto bg-surface-subtle/40 px-5 py-4">
                    ...
                    <div key={message.id} className={cn("flex", fromCustomer ? "justify-start" : "justify-end")}>
                      ...
                      >
                        ...
                        <p
                          className={cn(
                            "mt-1 text-[11px]",
                            fromCustomer || message.author === "assistant" ? "text-ink-faint" : "text-white/70",
                          )}
                        >
                          {message.authorName ?? (fromCustomer ? "Customer" : message.author)} · {formatTime(message.createdAt)}
                        </p>
                      </div>
                    </div>
                  );
              ...
              {canReply ? (
                <form className="flex items-end gap-2 border-t border-line px-4 py-3" onSubmit={handleSend} noValidate>
                  <textarea
                    value={reply}
