...
import { ArrowLeft, Copy, KeyRound, MessagesSquare } from "lucide-react";
...
export function ProductPanel(props: ProductPanelProps) {
  ...
  const totalUsage = usage?.metrics.reduce((sum, metric) => sum + metric.used, 0) ?? 0;
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-xl font-semibold tracking-tight text-ink">{product.displayName}</h2>
            {product.planName ? <Badge tone="brand">{product.planName}</Badge> : <Badge>No plan</Badge>}
            {product.planCode ? (
          ...
          <p className="mt-1 max-w-2xl text-[13px] text-ink-secondary">{product.description || product.productSlug}</p>
        </div>
        {granted ? (
          <Link
            href={`${props.conversationsBase}/${product.productSlug}/conversations`}
            className="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-line bg-surface px-3 py-2 text-[13px] font-medium text-ink transition-colors hover:bg-surface-subtle"
          >
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-line bg-line md:grid-cols-5">
        ...
        <MiniStat
          label="Monthly"
          value={product.priceMonthly !== null && product.currency ? formatCurrency(product.priceMonthly, product.currency) : "—"}
        />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="shadow-sm border-line bg-surface h-fit">
          <CardHeader
            title="Plan & billing"
            description={isPlatformAdmin ? "Choose a plan and control access." : "Your current plan and pricing."}
          />
          <div className="space-y-4">
            <div className="flex items-baseline justify-between">
              <span className="text-[13px] text-ink-muted">Current plan</span>
              <span className="text-sm font-medium text-ink">{product.planName ?? "Unassigned"}</span>
            </div>
            ...
            {isPlatformAdmin ? (
              <div className="flex flex-wrap items-end gap-3">
                <Field label="Plan" htmlFor={`plan-${product.productSlug}`} className="w-full max-w-xs">
                  <select
                  ...
                  >
                    ...
                    {product.availablePlans.map((plan) => (
                      <option key={plan.code} value={plan.code}>
                        {plan.name} — {formatCurrency(plan.priceMonthly, plan.currency)}/mo
                      </option>
                    ))}
                  </select>
                </Field>
                {product.planCode ? (
                  <Button type="button" variant="outline" size="sm" loading={props.savingPlan} onClick={props.onStatusToggle}>
            ...
            {product.scopes.length > 0 ? (
              <div>
                <p className="mb-2 text-xs font-medium uppercase tracking-wide text-ink-faint">Access scopes</p>
        ...
        <Card className="shadow-sm border-line bg-surface h-fit">
          <CardHeader title="Usage" description={usage ? `Billing period ${usage.period}` : "Current billing period"} />
          {usage ? (
            <div className="space-y-4">
              <div className="flex items-baseline justify-between border-b border-line pb-4">
                <span className="text-[13px] text-ink-muted">Estimated cost</span>
                <span className="text-sm font-medium text-ink">{formatCurrency(usage.estimatedCost, usage.currency)}</span>
              </div>
      ...
      <Card className="shadow-sm border-line bg-surface">
        ...
        {props.createdToken ? (
          <Alert tone="warning" title="Copy this access key now" className="mb-4">
            ...
            <div className="mt-3 flex gap-2">
              <Button type="button" variant="outline" size="sm" onClick={props.onCopyToken}>
                <Copy className="h-4 w-4" aria-hidden="true" />
                Copy key
              </Button>
              <Button type="button" variant="ghost" size="sm" onClick={props.onDismissCreated}>
                Dismiss
              </Button>
            </div>
          </Alert>
        ...
        {tokens.length === 0 ? (
          <p className="text-sm text-ink-muted">
            {granted ? "No keys yet. Create one to connect the product." : "Assign a plan to issue keys."}
          </p>
        ) : (
              ...
              <li key={token.id} className="flex items-center justify-between gap-4 px-4 py-3">
                <div className="min-w-0">
                  ...
                  {token.allowedDomains.length > 0 ? (
                    <p className="mt-0.5 truncate text-xs text-ink-faint">{token.allowedDomains.join(", ")}</p>
                  ) : (
                    <p className="mt-0.5 text-xs text-danger">No domains — widget blocked</p>
                  )}
                </div>
                <div className="flex items-center gap-3">
                  <Badge tone={token.revokedAt ? "danger" : "success"}>{token.revokedAt ? "Revoked" : "Active"}</Badge>
                  {isPlatformAdmin && !token.revokedAt ? (
                    <Button type="button" variant="outline" size="sm" onClick={() => props.onRevoke(token.id)}>
                      Revoke
                    </Button>
                  ) : null}
                </div>
        ...
        {isPlatformAdmin && granted ? (
          <form className="flex flex-wrap items-end gap-3 border-t border-line pt-4" onSubmit={props.onCreateToken} noValidate>
            <Field label="New key name" htmlFor={`token-${product.productSlug}`} className="w-full max-w-xs">
              <Input
                id={`token-${product.productSlug}`}
                value={props.tokenName}
                placeholder="Production"
                onChange={(event) => props.onTokenNameChange(event.target.value)}
              />
            </Field>
            <Field
            ...
            >
              <Input
                ...
                onChange={(event) => props.onTokenDomainsChange(event.target.value)}
              />
            </Field>
