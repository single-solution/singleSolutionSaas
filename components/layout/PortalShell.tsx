...
import { PortalUtilities } from "@/components/layout/PortalUtilities";
import { getHeaderNavigation } from "@/components/layout/portalNavigation";
...
export function PortalShell({
  ...
  const navigationItems = getHeaderNavigation(user.isPlatformAdmin);
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-surface-muted font-sans p-2 sm:p-3">
      ...
      <div className="mx-auto flex w-full min-h-0 max-w-[1920px] flex-1 flex-col gap-2 sm:gap-3">
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "group flex shrink-0 items-center gap-2 rounded-md px-2.5 py-2 text-[13px] font-medium transition-colors duration-150 min-h-11",
                    active
        ...
        <main
          id="main-content"
          className="flex min-h-0 flex-1 flex-col overflow-y-auto rounded-xl border border-line bg-surface shadow-sm lg:flex-row lg:overflow-hidden"
        >
          <div className="min-w-0 flex-1 p-4 sm:p-5 lg:min-h-0 lg:overflow-y-auto lg:p-6">
            <div className="w-full motion-safe:animate-fade-in-up">
              {children}
            </div>
          </div>
          <PortalUtilities user={user} />
        </main>
      </div>
    </div>
  );
