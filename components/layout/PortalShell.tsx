...
export function PortalShell({
  ...
  /** Optional inner sidebar rendered inside the main panel (e.g. contextual nav). */
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-surface-muted font-sans p-3 sm:p-4">
      <a
        href="#main-content"
        ...
        <header className="z-sticky flex h-14 shrink-0 items-center gap-2 rounded-xl border border-line bg-surface px-3 shadow-sm sm:gap-4 sm:px-4">
          <Link href="/" className="flex shrink-0 items-center gap-2.5">
            ...
            <div className="hidden flex-col sm:flex">
              <span className="text-[13px] font-semibold leading-none tracking-tight text-ink">Single Solution</span>
              <span className="mt-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-600">{roleLabel}</span>
            </div>
          </Link>

          <span className="hidden h-6 w-px bg-line sm:block" aria-hidden="true" />

          <nav className="flex flex-1 items-center gap-1 overflow-x-auto no-scrollbar" aria-label="Main navigation">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "bg-surface-subtle text-ink" : "text-ink-secondary hover:bg-surface-subtle hover:text-ink",
                  )}
                >
                  <Icon
                    className={cn(
          ...
          <div className="shrink-0">
            <ProfileMenu user={user} settingsHref="/settings" onSignOut={onSignOut} />
          </div>
        </header>
