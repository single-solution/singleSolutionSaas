...
import { Award, Gift, Crown, Building2, Globe, ArrowUpRight, Zap } from 'lucide-react';
...
export default function GuestLanding() {
	...
	return (
		<div className="space-y-6 max-w-6xl pb-12">
			<PageHeader
				title="Loyalty & Rewards Engine • Operations Hub"
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<StatCard title="Active Rewards Storefronts" value={totalWebsites.toString()} trend="Live Embedded Pills" />
				<StatCard title="Points Issued (24h)" value="84,200 pts" trend="+24.1% volume" />
				<StatCard title="Vouchers Redeemed" value="412" trend="High Conversion" />
			</div>
				...
				<div className="space-y-3">
					...
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
						{(features.length > 0
							? features
							: [
									{ id: 'points_ledger', name: 'Points & Transaction Ledger', hourlyPrice: 0.008, category: 'Core' },
									{ id: 'vip_tiers', name: 'VIP Dynamic Tier Multipliers', hourlyPrice: 0.015, category: 'Tiers' },
									{ id: 'vouchers', name: 'Instant Coupon & Voucher Vault', hourlyPrice: 0.007, category: 'Rewards' },
								]
						).map((f) => (
							<div key={f.id} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1">
								...
								<div className="text-[11px] text-slate-500">
									Feature Key: <code className="font-mono text-amber-600">{f.id}</code>
								</div>
							</div>
						))}
					</div>
