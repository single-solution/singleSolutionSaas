...
export default function WebhooksIntegration() {
	const { portalUrl, activeStore, stores, hasStoreFeature, webhookConfig, saveWebhook, toggleFeature } = useStorefront();
	...
	if (!hasStoreFeature('custom_webhooks')) {
		return (
			<FeatureLockScreen
				featureName="Real-Time Outbound Webhooks"
				creditCost={25}
				...
				onActivate={() => toggleFeature('custom_webhooks', 'enable')}
			/>
		);
	}
