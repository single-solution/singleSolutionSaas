...
export default function SearchAnalytics() {
	const { activeStore, analyticsData, hasStoreFeature, toggleFeature } = useStorefront();
	const [activeTab, setActiveTab] = useState('ALL');

	if (!hasStoreFeature('search_analytics')) {
		return (
			<FeatureLockScreen
				featureName="Search Query Intelligence"
				creditCost={25}
				desc="Customer search query forensics, zero-result missed sale tracking, and catalog discovery demand."
				onActivate={() => toggleFeature('search_analytics', 'enable')}
			/>
		);
	}
