...
import { Activity, Save, CheckCircle2, Zap, ShieldCheck, Send, ExternalLink, Lock, Check, Globe, Building2 } from 'lucide-react';
...
export default function Ga4Integration() {
	const { activeStore, stores, hasStoreFeature, ga4Config, saveGa4 } = useStorefront();
	...
	if (!activeStore || stores.length === 0) {
		return (
			<div className="space-y-6 antialiased text-slate-900 max-w-4xl">
				<PageHeader title="Google Analytics 4 Server Sync" subtitle="Server-side GA4 Measurement Protocol pipeline" />
				<Card>
					<div className="py-16 px-4 text-center space-y-4 max-w-md mx-auto">
						...
						<div className="space-y-1.5">
							...
							<p className="text-xs text-slate-500 leading-relaxed">
								Register a merchant store in the Master Portal to configure Google Analytics 4 Measurement Protocol.
							</p>
						</div>
						<div className="pt-2">
							<a
	...
	if (!hasStoreFeature('ga4_measurement')) {
		return (
			<FeatureLockScreen
				featureName="Google Analytics 4 & Tag Manager Server Sync"
				creditCost={30}
