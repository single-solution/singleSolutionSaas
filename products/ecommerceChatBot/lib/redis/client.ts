...
import { isProductionEnvironment, loadEnvironment } from "@/lib/env";
...
declare global {
  var __chatbotRedisClient: RedisCommands | undefined;
  var __chatbotRedisState: RedisRuntimeState | undefined;
}

function resolveUpstashConfig(): { url: string; token: string } | null {
  const environment = loadEnvironment();
  if (!environment.upstashRedisRestUrl || !environment.upstashRedisRestToken) {
    return null;
  }
  return {
...
export function getRedisRuntimeState(): RedisRuntimeState {
  return (
    globalThis.__chatbotRedisState ?? {
      mode: "memory",
      available: true,
    }
  );
...
export function getRedisClient(): RedisCommands {
  ...
  if (globalThis.__chatbotRedisClient) {
    return globalThis.__chatbotRedisClient;
  }

  const config = resolveUpstashConfig();
  if (config) {
    const upstash = new Redis({ url: config.url, token: config.token });
    globalThis.__chatbotRedisClient = {
    ...
    globalThis.__chatbotRedisState = { mode: "upstash", available: true };
    return globalThis.__chatbotRedisClient;
  }

  globalThis.__chatbotRedisClient = new BoundedMemoryRedis();
  globalThis.__chatbotRedisState = { mode: "memory", available: true };
  return globalThis.__chatbotRedisClient;
}
